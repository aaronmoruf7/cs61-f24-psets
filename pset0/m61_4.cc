...
void* m61_malloc(size_t sz, const char* file, int line) {
    ...
    if (sz == 0) {
        ++gstats.ntotal;
        return nullptr;  // According to the specification, return nullptr for zero-sized allocation
    }
    // Check for overflow before proceeding
    if (sz == 0 || sz > SIZE_MAX - 8) {  // Check if adding the boundary bytes causes overflow
        ++gstats.nfail;          // Increment fail count
        gstats.fail_size += sz;   // Add the size of the failed allocation
